...
const InputNumberCustoms = props => {
    let { readOnly, label, onChange, value, addonBefore, className, classNameInput,
        style, min, max, step, classNameSlider, titleClassName, format,
                ...
                {classNameGroupInput ?
                    <div className={classNameGroupInput}>
                        <InputNumber
                            ...
                            step={step ? step : 1}
                            formatter={value => format ? `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, '.') : value}
                            parser={value => format ? value.replace(/\$\s?|[.]/g, '') : value}
